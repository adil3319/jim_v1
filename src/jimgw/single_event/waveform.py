...
def get_inspiral_phase_lm_1(fM_s: Array, theta: Array, coeffs: Array) -> Array:
    """
...
def get_IIb_raw_phase_lm_1(fM_s: Array, theta: Array, coeffs: Array, f_RD, f_damp) -> Array:
    m1, m2, _, _ = theta
    m1_s = m1 * gt
    m2_s = m2 * gt
    M_s = m1_s + m2_s
...
def get_inspiral_Amp_lm_1(fM_s: Array, theta: Array, coeffs: Array) -> Array:
...
def get_IIb_Amp_lm_1(fM_s: Array, theta: Array, coeffs: Array, f_RD, f_damp) -> Array:
    m1, m2, _, _ = theta
    m1_s = m1 * gt
    m2_s = m2 * gt
    M_s = m1_s + m2_s
...
def Phase_lm_1(f: Array, theta: Array, coeffs: Array, transition_freqs: Array) -> Array:
    ...
    phi_IIb = (
        get_IIb_raw_phase_lm_1(f * M_s, theta, coeffs, f_RD, f_damp)
        + a0
        + a1_correction * (f * M_s)
    )
...
def gen_IMRPhenomD(f: Array, params: Array, f_ref: float):
    ...
    h0T = _gen_IMRPhenomD(f, theta_intrinsic, theta_extrinsic, coeffs, f_ref)
    h0S = _gen_IMRPhenomD_lm_1(f, theta_intrinsic, theta_extrinsic, coeffs, f_ref)
    return [h0T,h0S]



def gen_IMRPhenomD_hphchb_lm_1(f: Array, params: Array, f_ref: float):
    """
    Generate PhenomD frequency domain waveform following 1508.07253.
    vars array contains both intrinsic and extrinsic variables
    theta = [Mchirp, eta, chi1, chi2, D, tc, phic]
...
class RippleIMRPhenomD_ScalarTensor(Waveform):
    ...
    def __call__(
       ...
       # hp, hc, hb = gen_IMRPhenomD_hphchb(frequency, theta, self.f_ref)
        ...
        # uT=(jnp.pi*(theta[0])*frequency*cf/c)**(1/3)
        # uS=(jnp.pi*(theta[0])*frequency*cf/c)**(1/3) # cf =1.4765e3 m
        a,b=-2,-7
        a1 = -2
        hpT = hp*(1+theta[-3]*uT**a)*jnp.exp(1.0j*theta[-2]*uT**b)
        hcT = hc*(1+theta[-3]*uT**a)*jnp.exp(1.0j*theta[-2]*uT**b)
        hbT = hb*theta[-1]*(uS**a1)*jnp.exp(1.0j*theta[-2]*uS**b)
